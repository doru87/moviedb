...
export const SearchBarFilme = (props) => {
    ...
    React.useEffect(() => {
      let unmounted = false;
      fetch(`https://api.themoviedb.org/3/movie/popular?api_key=${apiKey}&language=ro&page=1`)
    ...
    const handleClick = (event) => {
        const results = movieList.filter(latest =>
            latest.original_title.toLowerCase().includes(searchMovie)
          );
